...
class FibonacciEmaStrategy:
    ...
    def __init__(self, config: dict):
        ...
        logger.info(
            f"FIBONACCI_EMA initialized: ema_period={self.ema_period}, swing_lookback={self.swing_lookback}, "
    ...
    def _detect_swings(self, df: pd.DataFrame) -> tuple:
        ...
        rolling_max = (
            df["high"].rolling(window=window_size, center=True, min_periods=window_size).max()
        )
        ...
        rolling_min = (
            df["low"].rolling(window=window_size, center=True, min_periods=window_size).min()
        )
    ...
    def generate_signals(self, df: pd.DataFrame) -> pd.Series:
        ...
        valid_swings = (
            df_sim["current_swing_high"].notna()
        ...
        df_sim.loc[valid_swings, "fib_level_1"] = df_sim.loc[valid_swings, "current_swing_low"] + (
            price_range[valid_swings] * self.fib_level_1
        )
        df_sim.loc[valid_swings, "fib_level_2"] = df_sim.loc[valid_swings, "current_swing_low"] + (
            price_range[valid_swings] * self.fib_level_2
        )
        ...
        cross_below_ema = (
            (prev_price >= prev_ema) & (df_sim["close"] < df_sim["ema"]) & df_sim["ema"].notna()
        )

        # Sell Signal 2: Price reaches swing high (take profit)
        at_swing_high = (df_sim["close"] >= df_sim["current_swing_high"]) & df_sim[
            "current_swing_high"
        ].notna()
        ...
        logger.info(
            f"FIBONACCI_EMA generated {buy_count} buy signals and {sell_count} sell signals"
        )
