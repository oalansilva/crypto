...
class CruzamentoMedias:
    ...
    def __init__(self, config: dict):
        ...
        self.media_curta = (
            config.get("media_curta") or config.get("sma_short") or config.get("ema_short") or 6
        )
    ...
    def generate_signals(self, df: pd.DataFrame) -> pd.Series:
        ...
        mediabuy = (df_sim["media_curta"] > df_sim["media_longa"]) & (
            df_sim["media_curta"].shift(1) <= df_sim["media_longa"].shift(1)
        )
        mediabuyinter = (df_sim["media_curta"] > df_sim["media_inter"]) & (
            df_sim["media_curta"].shift(1) <= df_sim["media_inter"].shift(1)
        )

        # Crossunder: current < target AND previous >= target
        mediasell = (df_sim["media_curta"] < df_sim["media_inter"]) & (
            df_sim["media_curta"].shift(1) >= df_sim["media_inter"].shift(1)
        )
        ...
        logger.info(
            f"CRUZAMENTOMEDIAS generated {buy_condition.sum()} buy signals and {sell_condition.sum()} sell signals"
        )
