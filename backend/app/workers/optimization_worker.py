...
def _get_signals_for_params(
    strategy_name: str, params_tuple: Tuple[Tuple[str, Any]], timeframe: str
) -> pd.DataFrame:
    """
...
def evaluate_combination(config: Dict, combination: Tuple, keys: List[str]) -> Dict:
    ...
    try:
        ...
        # This call is CACHED
        df_with_signals = _get_signals_for_params(
            str(strategy_name), strat_params_tuple, str(timeframe)
        )
        ...
        if not trades:
            ...
            try:
                ...
                metrics["trade_concentration_top_10_pct"] = calculate_trade_concentration(
                    trades, top_n=10
                )
