    ...
    def should_run(self, now: datetime, material_change: bool = False) -> bool:
        ...
        if self.suppressed_since:
            ...
            if elapsed > timedelta(minutes=self.suppression_timeout_minutes):
                logger.info(f"Suppression timeout reached ({self.suppression_timeout_minutes}min), forcing turn")
                self.suppressed_count = 0
                self.suppressed_since = None
                return True
...
class WorkflowPollingSuppressor:
    ...
    def capture_state(self, db: Session) -> WorkflowStateSnapshot:
        ...
        active_changes = {
            ...
            if c.status not in ("archived", "canceled", "done")
        }
        
        # Get work item states
        work_items = db.query(WorkItem).all()
        work_item_states = {
            wi.id: wi.state.value for wi in work_items
        }
        
        # Get approvals
        approvals = db.query(WorkflowApproval).all()
        approval_states = {
            a.id: a.state.value for a in approvals
        }
    ...
    def detect_material_changes(
        self, 
        old_state: Optional[WorkflowStateSnapshot], 
    ...
    def should_scheduler_run(self, db: Session) -> tuple[bool, Dict[str, Any]]:
        ...
        metadata = {
            ...
            "suppressed_since": self._state.suppressed_since.isoformat() if self._state.suppressed_since else None,
        ...
        if should_run:
            ...
            logger.info(f"Scheduler decision: RUN (material_changes={material_changes}, state_changed={state_changed})")
        else:
            self._state.record_suppressed(now)
            logger.info(f"Scheduler decision: SUPPRESS (suppressed_count={self._state.suppressed_count})")
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "suppressed_count": self._state.suppressed_count,
            "last_turn_at": self._state.last_turn_at.isoformat() if self._state.last_turn_at else None,
            "suppressed_since": self._state.suppressed_since.isoformat() if self._state.suppressed_since else None,
            "last_hash": self._state.last_turn_hash,
