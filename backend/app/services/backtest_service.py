...
class BacktestService:
    ...
    def run_backtest(
        self, config: dict, progress_callback=None, job_id: str = None, resume: bool = False
    ) -> dict:
        """
        ...
        for timeframe in target_timeframes:
            ...
            try:
                df = self.loader.fetch_data(
                    symbol, timeframe, since, until, progress_callback=loader_callback
                )
            ...
            for i, strategy_config in enumerate(strategies):
                ...
                # Unique result key
                strategy_key = (
                    f"{base_name} ({timeframe})" if len(target_timeframes) > 1 else base_name
                )
                ...
                try:
                    ...
                    if equity_curve is not None and not equity_curve.empty:
                        metrics = calculate_metrics(
                            equity_curve, backtester.trades, config.get("cash", 10000)
                        )
                    ...
                    drawdown = (
                        ((equity_curve["equity"] - running_max) / running_max * 100)
                        .fillna(0)
                        .tolist()
                    )
                    ...
                    indicators = []
                    if (
                        hasattr(backtester, "simulation_data")
                        and backtester.simulation_data is not None
                    ):
                        ...
                        for col in indicator_cols:
                            ...
                            if not series_data.empty:
                                formatted = series_data.rename(
                                    columns={"timestamp_utc": "time", col: "value"}
                                ).copy()
    ...
    def _run_optimization(
        ...
        return self._run_single_strategy_optimization(
            config, progress_callback, job_id=job_id, resume=resume
        )
    ...
    def _run_multi_strategy_optimization(self, config: dict, progress_callback=None) -> dict:
        ...
        for strat_idx, strategy_config in enumerate(strategies):
            base_name = (
                ...
                else strategy_config.get("name", f"Strategy{strat_idx}")
            )
        ...
        for strat_result in all_strategy_results:
            if strat_result["best"] and strat_result["best"]["metrics"]["total_pnl"] > best_pnl:
                ...
                overall_best = {"strategy": strat_result["strategy"], **strat_result["best"]}
    ...
    def _run_single_strategy_optimization(
        self,
        config: dict,
        ...
        base_name = (
            strategy_config
            if isinstance(strategy_config, str)
            else strategy_config.get("name", "Strategy")
        )
        ...
        if resume and job_id:
            ...
            if state:
                ...
                logger.info(
                    f"Resuming optimization from index {start_index}. Found {existing_count} previous results in database."
                )
        ...
        batch_size = 50
        chunks = [
            remaining_combinations[i : i + batch_size]
        ...
        try:
            with ProcessPoolExecutor(
                max_workers=max_workers, initializer=opt_worker.init_worker, initargs=(data_map,)
            ) as executor:
                # Submit all chunks
                ...
                for future in as_completed(future_to_chunk_idx):
                    ...
                    if job_id and job_manager.should_pause(job_id):
                        ...
                        return {
                            "status": "paused",
                            "progress": f"{current_processed}/{total_steps}",
                            "results": top_results,
                        }
                    ...
                    try:
                        ...
                        if progress_callback:
                            progress_callback(
                                f"Optimizing... {current_processed_count}/{total_steps}", pct
                            )
            ...
            if "error" in best_scout:
                ...
                try:
                    ...
                    if equity_curve is not None and not equity_curve.empty:
                        ...
                        # Add enhanced
                        print(
                            f" DEBUG: About to call _calculate_enhanced_metrics with {len(backtester.trades)} trades"
                        )
        ...
        if job_id:
            final_state = {
                ...
                "status": "COMPLETED",
                "config": config,
                "progress": {"current_iteration": total_steps, "total_iterations": total_steps},
                    ...
                    markers.append(
                        {
                            ...
                            "position": "aboveBar",
                            "color": (
                                "#e91e63" if not is_win else "#00E676"
                            ),  # Green for win, Pink for loss
                            "shape": "arrowDown",
                            "text": "SELL",
                        }
                    )
    ...
    def _calculate_benchmark(self, df, initial_capital: float) -> dict:
        ...
        try:
            ...
            return {"return_pct": return_pct, "final_value": final_value, "equity": equity}
        except:
            return {}
    ...
    def _calculate_enhanced_metrics(
        ...
        try:
            ...
            try:
                enhanced["trade_concentration_top_10_pct"] = calculate_trade_concentration(
                    trades, top_n=10
                )
            except Exception as e:
