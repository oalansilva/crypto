...
KNOWN_DEFAULTS = {
    ...
    "bbands": {
        "length": 20,
        "std": 2.0,
    },  # Market standard: 20 period, 2 std dev (Bollinger's original)
...
def generate_optimization_range(
    ...
    if any(
        keyword in param_lower
        for keyword in ["threshold", "overbought", "oversold", "upper", "lower", "rsi_u", "rsi_d"]
    ):
    ...
    if isinstance(default_value, int) or (
        isinstance(default_value, str) and default_value.isdigit()
    ):
