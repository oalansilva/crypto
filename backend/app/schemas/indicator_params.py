...
def estimate_total_tests(indicator_schema: IndicatorSchema) -> int:
    ...
    for param in indicator_schema.parameters.values():
        if param.optimization_range:
            range_size = (
                int(
                    (param.optimization_range.max - param.optimization_range.min)
                    / param.optimization_range.step
                )
                + 1
            )
            total += range_size
    ...
    sl_range_size = (
        int(
            (sl_param.optimization_range.max - sl_param.optimization_range.min)
            / sl_param.optimization_range.step
        )
        + 1
    )
    total += sl_range_size
