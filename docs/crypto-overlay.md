...
- Runbook de processo: skill `.cursor/skills/covenant-flow/` **neste repo** (GitHub). Overlay cripto (board, ports, Drive, PostgreSQL, release-guard, `./restart`) fica neste arquivo. Lei humana: `rules.md`. Always-on curto: `.cursor/rules/harness.mdc`.
- **Em qualquer pedido de release, publicação, lote, deploy, produção ou operação que possa afetar DEV/PROD, carregue também `.cursor/skills/covenant-flow-environments/`** (mapa DEV/PROD e deploy PROD antes de `Pronto`).
...
- Se uma regra geral de processo precisar mudar, edite `.cursor/skills/covenant-flow/` neste repo (não dual-write hermes/Codex).

## TL;DR

- **Skills:** `.cursor/skills/covenant-flow`, `covenant-flow-environments`, `github-project-board` (arquivos reais no GitHub). OpenSpec: `.cursor/skills/openspec-*`. Design: `.agents/skills/design-critic`.
...
- **OpenSpec no card:** Gist `crypto openspec <change>` **superset** do issue; republicar o mesmo Gist. Helper: `.cursor/skills/covenant-flow/scripts/publish-openspec-card-artifacts.sh`.
- **Banco:** PostgreSQL (`DATABASE_URL`, `WORKFLOW_DATABASE_URL`). Não usar SQLite em runtime/QA.
- **Release:** `covenant-flow` + `covenant-flow-environments`; `scripts/release-guard pre|post`; deploy PROD obrigatório antes de `Pronto`. Detalhe das 12 colunas, gates e higiene: skill `covenant-flow` + `rules.md` §3.
...
- **Regra de não regressão de status:** depois que um card estiver em `Done`, nunca mova de volta para `Em desenvolvimento` durante homologação, archive, commit, PR ou merge — exceptua **somente** T18 humano (`nao_homologar`, Alan) com motivo visível no issue (`Não homologar:` + texto). Agent, archive, commit, PR, merge e closeout continuam proibidos nessa volta. Se aparecer falha, ajuste necessário ou reteste, corrija e reteste mantendo o status atual. O card só avança: `Done` -> `Homologado` -> `Pronto`, salvo a mesma T18 humana com motivo (volta a `Em desenvolvimento`). Homologado não tem aresta inversa.
...
- **Regra de checks em execução:** seguir `covenant-flow`; no cripto, isso vale para testes locais, `openspec validate`, build e CI antes de `Code Review`, `QA`, `Done`, release/lote, commit, PR ou merge. Check `running`, `cancelled` ou skip sem dispensa autorizada nao e evidencia final.
...
- **Regra de worktree limpo no fechamento:** seguir `covenant-flow`; no cripto, trabalho de outra change deve ir para branch/worktree própria e a integração padrão acontece em `develop` antes de produção.
- **Regra de varredura da release:** seguir o inventario/classificacao de `covenant-flow`; no cripto, integre o que deve entrar em `develop`, publique em `main` via PR/merge manual quando permitido e só então limpe branches/worktrees.
...
- **Regra de evidência de deploy PROD no guard:** `release-guard post` (e `pre` após a publicação) exige `PROD_DEPLOY_EVIDENCE` com `<commit-publicado> services=<svcs> url=<url-publica>` antes de mover cards para `Pronto`; sem evidência, o guard falha em modo estrito. No `pre` antes do merge a evidência ainda não é exigida (deploy ocorre após o merge). Com overlay PROD presente, `services=` deve listar **todos** os units da janela do publish (overlay `services` − `oneshot_services`, com ou sem sufixo `.service`); omissão é blocker; nomes oneshot a mais são aviso; unit da janela **activo** com arranque anterior à janela deste deploy é blocker; unit parado com `release.health_url` ok não bloqueia o fecho (incidente à parte).
...
- **Regra anti-stash órfão:** seguir `covenant-flow`; nenhuma release/lote pode terminar com stash novo ou antigo sem classificacao explicita.
...
- **OpenSpec no card antes de implementar:** seguir `covenant-flow`; neste repo, preferir Gist secreto descrito como `crypto openspec <change>` e comentario no issue/card.
...
No **Cursor**, as skills do projeto carregam de `.cursor/skills/` (`covenant-flow`, `covenant-flow-environments`, `github-project-board`, OpenSpec, kaizen) e `.agents/skills/` (design-critic, impeccable, playwright-cli). Os commands `/opsx-*` e `/kaizen` vivem em `.cursor/commands/`. O hook Impeccable está em `.cursor/hooks.json` e chama `.agents/skills/impeccable/scripts/hook.mjs`. Invocar `/opsx-new` ou a skill `openspec-new-change` tem o mesmo efeito.
...
Antes de executar `/opsx:apply` em qualquer change vinculada a card/issue, siga `covenant-flow` e publique os artefatos OpenSpec no card. Convencao local do Gist: descricao `crypto openspec <change>` e comentario no card do Project 1.
...
publicar artifacts OpenSpec no card
  -> seguir covenant-flow; no cripto, usar Gist `crypto openspec <change>` ou links permanentes
...
- **Impeccable (pipeline formal via skill):** para `UI impact: affected`, executar a skill local em `context -> shape -> prototype -> critique -> audit -> targeted fixes -> polish -> browser gate`. Avaliação (rubrica, dual critic, detector, browser, zero P0/P1) permanece intacta; `DESIGN.md` sem sobrescrita. Emissão publicada (chat e seções Impeccable/Design Critique de `design.md`) = bullets P0–P3 + disposition + verdict; relatório longo em `.impeccable/critique/` (git-tracked, link no card; apply/review não lêem). Assessment A e B usam `Task` isolada no **mesmo modelo do chat**, prompt autocontido, sem transcript do pai; podem escrever só `.impeccable/critique/**`. Snapshot vazio ⇒ `BLOCKED`. Vale nos dois clientes (Cursor e Grok via MUST Read do canônico). Para `UI impact: none`, registrar Impeccable como `N/A` com justificativa e manter todos os gates.
...
- **Terceiro caminho (card #617):** se o push do archive/closeout em `refs/heads/develop` for recusado por proteção (`qa-gate` ou equivalente), **mesmo** com pacote só Homologado, não force bypass admin nem “publique o archive em `develop` primeiro”. Crie `release-YYYY-MM-DD` a partir de `origin/develop`, commit o archive (e docs/kaizen do pacote) nessa branch, rode `scripts/release-guard pre` com HEAD em `release-*` (PASS **não** exige archive já em `origin/develop`), abra PR `release-* → main`, merge, deploy PROD, depois sync explícito `main → develop` (um PR de merge normal) e reexecute `scripts/release-guard post` até `origin/main` ser ancestral de `origin/develop`. A ordem canônica #518 (kaizen, PR documental, `RELEASE_BRANCHES`, `post`) permanece; este caminho **insere** o uso de `release-*` + sync, não a substitui.
...
Seguir `covenant-flow` Visual QA. Quem compara pixels e o Playwright; o agente nao precisa ler todos os PNGs para “passar” o teste.
...
PRESERVED_BRANCHES="${PRESERVED_BRANCHES:-}" \
scripts/release-guard pre
# pre em release-* com PR de código corre npm ci + tsc -b --noEmit (sem gravar frontend/dist tracked); se falhar, não abra o PR
...
# sync main → develop — OBRIGATÓRIO: um PR de merge normal (recusar #926 e #913+#914+#915) antes do post final
gh pr create --base develop --head main --title "sync: main → develop após release-YYYY-MM-DD" --body "Containment: origin/main ancestral de origin/develop para release-guard post"
# merge do sync, então:
scripts/release-guard post
# se o post falhar porque develop não contém produção, complete o merge normal e reexecute o post (extra na develop = aviso)
```
