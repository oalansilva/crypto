...
Cliente: Cursor Agent / Grok Build / OpenCode. Canónico: `.cursor/skills/implantar/` neste produto.
...
```bash
/path/to/covenant-flow/install.sh --pin v1.0.0
```

Copia: nucleus (`.cursor/process-fsm.yaml`, `scripts/process-fsm/`), três adapters (`.cursor/` `.grok/` `.opencode/`), `.agents/skills/` (`impeccable`, `design-critic`, `playwright-cli`), helpers (`publish-openspec-card-artifacts.sh`, `release-guard` genérico), `AGENTS.md` gerado do overlay. Grava `pin` no overlay. **Não** sobrescreve o Markdown `overlay_doc`.
