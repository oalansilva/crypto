...
- **Exceção explícita — planejamento de design (gate Design):** a preparação de design (especificações, crítica e protótipo) é feita pelo subagent `design-planner` (`.opencode/agent/design-planner.md`) com `model: openai/gpt-5.6-sol` fixo (provider OpenAI via OAuth) e `reasoningEffort: high` — segunda exceção à herança de modelo, ao lado do `vision`. A sessão principal permanece no Go/flash, consolida a entrega e move `Design -> Aprovação de Design`; os critics do Impeccable (Assessment A/B) herdam o modelo da sessão de design designada (`design-planner`), com igualdade observável obrigatória — sem ela, `BLOCKED`. Pixels continuam sempre delegados ao `vision`. **Envelope de custo:** o frontier nunca roda como sessão principal (evidência: sessão de exploração com grok 4.6 queimou ~8,5M cache_read ≈ $3+); o `design-planner` recebe packet compacto e visa ≤ $1,00/card. Fallback operacional `opencode-go/grok-4.5` (effort high) só com autorização de Alan.
