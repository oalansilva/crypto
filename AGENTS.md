...
- **Opção 2 / contenção comportamental:** toda lane Luna recebe propriedade exata, ações permitidas/proibidas, limites externos e inventário antes/depois com digest. Sandbox e permission profile continuam observáveis, mas `danger-full-access` efetivo não bloqueia sozinho quando agente/modelo/effort/`fork_turns="none"` estão corretos; a lane só passa com auditoria independente de escopo. Isso reduz a garantia técnica: contenção comportamental não é isolamento forte do sistema operacional, e o risco residual deve aparecer no handoff.
...
- Depois da ativação, verificar agent type, modelo, effort, sandbox e permission profile efetivos quando a lane Luna for usada; `fork_turns="none"` é evidência separada do pedido/resultado de spawn controlado pelo orquestrador. Metadata de agente/modelo/effort/thread/sandbox/permissão ausente, conflitante ou divergente bloqueia; sandbox efetivo mais amplo que o pedido não bloqueia sozinho e ativa a contenção comportamental com risco residual explícito. O inspector local `inspect-agent-runtime.sh` prova somente os cinco campos runtime allowlisted e nunca prova `fork_turns`; sem o registro controlado de spawn a etapa bloqueia. Não existe fallback para Terra, built-in ou outro effort.
- Toda lane Luna exige estado relevante registrado antes do spawn e inspeção independente depois do retorno. Implementer só pode alterar caminhos atribuídos; reviewer deve manter **nenhuma mutação observada dentro do inventário obrigatório**: para cada repo/worktree nomeado, registrar `GIT_OPTIONAL_LOCKS=0` nos reads Git, `git worktree list`, HEAD/branch, refs/branches/tags, stash, config/hooks, tracked/untracked/ignored nos roots explicitamente inventariados e digests de conteúdo/links relevantes. Isso não significa zero mutação global: outros paths/repos do host, leituras/rede/ações externas sem API auditável e roots ignored excluídos por custo são risco residual declarado. Qualquer diferença observada ou exclusão não declarada bloqueia; reviewer não repara mutação. Release manager só pode agir no pacote Homologado e checklist externo autorizado.
...
- Usa `gpt-5.6-luna` / `max`, solicita `workspace-write` no perfil, sempre com `fork_turns="none"` e executa contenção comportamental mesmo se o runtime ampliar o sandbox. Antes/depois, somente caminhos atribuídos; commits, push, PR, board e serviços proibidos salvo autorização explícita. O handoff registra sandbox/permission observáveis e o risco residual de não haver isolamento do sistema operacional.
...
- Usa uma nova thread `gpt-5.6-luna` / `max`, solicita `read-only` no perfil e registra `fork_turns="none"` no pedido/resultado controlado pelo orquestrador; o inspector local não prova esse campo. Permanece read-only por comportamento, não por igualdade de sandbox. Antes/depois, para cada repo/worktree nomeado, compara o inventário obrigatório (worktree list, HEAD/branch, refs/branches/tags, stash, config/hooks, tracked/untracked/ignored nos roots declarados e digests); aceita somente **nenhuma mutação observada dentro do inventário obrigatório**. Isso não é zero mutação global: limites não observáveis ficam no risco residual. Qualquer diferença ou exclusão não declarada bloqueia e o reviewer não repara. Sandbox/permission observáveis e risco residual são reportados com segurança.
...
- Usa uma nova thread `gpt-5.6-luna` / `max`, solicita `danger-full-access`, sempre com `fork_turns="none"` e só atua sobre pacote Homologado explicitamente autorizado. Inventaria repositórios/worktrees/stashes e audita ações antes/depois; não altera código nem conteúdo fora do pacote. O sandbox amplo é risco residual, não isolamento forte do sistema operacional.
