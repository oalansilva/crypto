...
- **Exceção explícita — planejamento de design (gate Design):** a preparação de design **DEVE ser executada pelo `design-planner`** (`.opencode/agent/design-planner.md`) com `openai/gpt-5.6-sol`, `variant: high`, em todo card. A sessão principal cria somente o scaffold, coleta contexto e orquestra os estágios pelo guard `.opencode/plugin/design-gate-guard.ts`: `design_openspec_readonly` para OpenSpec sem Bash, `design_spawn_stage` como único spawn e `design_artifact_write` como única capacidade do author, sempre com packet selado, lease e path exato. Task direto, `opencode run --agent design-planner`, autoria normativa pela main ou processo OpenCode diferente de `1.18.18` invalidam a evidence e produzem `BLOCKED`. Assessment A/B usam sessões separadas do perfil `design-critic-readonly`, também Sol/high e zero-tool; bytes, lineage e runtime são reconciliados deterministicamente. A main valida/publica e pode mover somente `Design -> Aprovação de Design`; pixels continuam exclusivos do `vision`. Fallback operacional `opencode-go/grok-4.5` (effort high) só com autorização explícita de Alan.
...
- **Impeccable (pipeline formal via skill):** para `UI impact: affected`, executar a skill local em `context -> shape -> prototype -> critique -> audit -> targeted fixes -> polish -> browser gate`, registrar `Impeccable Brief`, `Impeccable Critique`, `Impeccable Audit` e `Impeccable Trace`, e manter `DESIGN.md` sem sobrescrita. Assessment A e B usam o perfil zero-tool `design-critic-readonly` em child sessions distintas, com packet idêntico e `openai/gpt-5.6-sol`/`variant: high` observados; sem igualdade e correlação verificáveis, o veredito é `BLOCKED`. Para `UI impact: none`, registrar Impeccable como `N/A` com justificativa e manter todos os gates.
