...
- **Regra de fechamento sem pendência:** uma release/lote só pode ser reportada como concluída depois que todo item relacionado ao pacote estiver resolvido ou explicitamente classificado. Não deixe para trás change OpenSpec homologada sem archive, branch/worktree temporária, stash, PR, card, documentação, Drive sync, arquivo modificado, arquivo não rastreado ou bloqueio de validação sem registrar destino: `integrado`, `arquivado`, `preservado com motivo` ou `bloqueado com evidência`.
...
- Antes de reportar a release como concluída, confirme que não ficou pendência relacionada ao pacote: changes OpenSpec arquivadas ou classificadas, branches/worktrees temporárias limpas ou preservadas com motivo, stashes classificados, docs/Drive sincronizados quando aplicável, cards atualizados, PR/CI resolvidos e `git status --short` sem sobras não explicadas.
