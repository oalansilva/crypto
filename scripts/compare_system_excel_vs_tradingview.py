"""
Script para comparar dados do Excel do Sistema com TradingView.

Baseado na imagem fornecida com 55 trades do sistema.
"""
...
# Dados do Sistema extraídos da imagem (primeiros trades visíveis)
# Baseado na descrição: formato "2026-01-04T00:00:00+00:00", preços com vírgula decimal
SYSTEM_TRADES_FROM_IMAGE = [
    # Trades mais recentes (visíveis no topo da imagem)
    {'entry_time': '2026-01-04T00:00:00+00:00', 'entry_price': 3127.11, 'exit_time': '2026-01-20T00:00:00+00:00', 'exit_price': 2942.61051, 'signal': 'Stop', 'return_pct': -6.04},
    {'entry_time': '2025-10-09T00:00:00+00:00', 'entry_price': 4525.72, 'exit_time': '2025-10-10T00:00:00+00:00', 'exit_price': 4258.70, 'signal': 'Stop', 'return_pct': -6.04},
    {'entry_time': '2025-09-16T00:00:00+00:00', 'entry_price': 4523.74, 'exit_time': '2025-09-22T00:00:00+00:00', 'exit_price': 4256.83, 'signal': 'Stop', 'return_pct': -6.04},
    {'entry_time': '2025-08-12T00:00:00+00:00', 'entry_price': 4223.22, 'exit_time': '2025-08-29T00:00:00+00:00', 'exit_price': 4511.20, 'signal': 'Close entry(s) order...', 'return_pct': 6.66},
    {'entry_time': '2025-07-10T00:00:00+00:00', 'entry_price': 2768.74, 'exit_time': '2025-08-03T00:00:00+00:00', 'exit_price': 3393.94, 'signal': 'Close entry(s) order...', 'return_pct': 22.40},
    {'entry_time': '2025-05-04T00:00:00+00:00', 'entry_price': 1833.52, 'exit_time': '2025-05-29T00:00:00+00:00', 'exit_price': 2681.61, 'signal': 'Close entry(s) order...', 'return_pct': 46.04},
    {'entry_time': '2024-11-07T00:00:00+00:00', 'entry_price': 2721.88, 'exit_time': '2024-12-19T00:00:00+00:00', 'exit_price': 3626.80, 'signal': 'Close entry(s) order...', 'return_pct': 33.05},
    {'entry_time': '2024-10-18T00:00:00+00:00', 'entry_price': 2605.79, 'exit_time': '2024-10-23T00:00:00+00:00', 'exit_price': 2452.04, 'signal': 'Stop', 'return_pct': -6.04},
    # Trade mais antigo visível
    {'entry_time': '2017-10-13T00:00:00+00:00', 'entry_price': 303.52, 'exit_time': '2017-10-18T00:00:00+00:00', 'exit_price': 285.61, 'signal': 'Stop', 'return_pct': -6.04},
]

# Dados completos do TradingView (já extraídos anteriormente)
...
def compare_trades(system_trades: List[Dict], tradingview_trades: List[Dict]):
    ...
    print("\n" + "="*100)
    print("COMPARACAO: Sistema (Excel) vs TradingView")
    print("="*100)
    ...
    for sys_trade in system_trades:
        ...
        if best_match and best_score >= 5:
            ...
            is_perfect = (
                ...
                best_match['exit_date_diff'] == 0 and
                best_match['exit_price_diff'] < 0.01 and
    ...
    if differences:
        ...
        for i, diff in enumerate(differences[:30], 1):
            sys_t = diff['sys']
            tv_t = diff['tv']
            print(f"\n[{i}] Sistema: {sys_t.get('entry_time', 'N/A')[:10]} @ ${sys_t.get('entry_price', 0):.2f} -> {sys_t.get('exit_time', 'N/A')[:10]} @ ${sys_t.get('exit_price', 0):.2f}")
            print(f"    TradingView #{tv_t.get('trade_num', '?')}: {tv_t.get('entry_date', 'N/A')} @ ${tv_t.get('entry_price', 0):.2f} -> {tv_t.get('exit_date', 'N/A')} @ ${tv_t.get('exit_price', 0):.2f}")
    ...
    if not_found:
        ...
        for i, trade in enumerate(not_found[:20], 1):
            print(f"  [{i}] Entry: {trade.get('entry_time', 'N/A')[:10]} @ ${trade.get('entry_price', 0):.2f}")
    
    print("\n" + "="*100)

if __name__ == "__main__":
    print("COMPARACAO: Dados Excel (Sistema) vs TradingView")
    print("\nComparando trades do sistema com dados do TradingView extraídos das imagens.")
    
    # Comparar com os dados visíveis na imagem
    compare_trades(SYSTEM_TRADES_FROM_IMAGE, TRADINGVIEW_ALL_TRADES)
    
    print("\n[NOTA] Esta comparação usa apenas os trades visíveis na imagem.")
    print("Para comparar todos os 55 trades:")
    print("1. Exporte o Excel completo para CSV")
    print("2. Use: python process_full_trades_comparison.py <arquivo.csv>")
