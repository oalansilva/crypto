#!/usr/bin/env python3
"""Revalidate issue #277 BTC/USDT 1d Long T0 Favorites with Deep Backtest."""
        ...
        json.dumps(
            {
                ...
                "benchmarks": {
                    key: value and value.get("favorite_id")
                    for key, value in payload["benchmarks"].items()
                    if key != "BENCHMARK_PARETO_SET"
                },
                "pareto_size": len(payload["benchmarks"]["BENCHMARK_PARETO_SET"]),
                "output": str(output),
            },
            ensure_ascii=False,
            sort_keys=True,
