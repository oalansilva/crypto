"""Generate OpenCode skill stubs for .cursor/skills (not .agents extras)."""
...
def stub_body(name: str, canonical_dir: str) -> str:
    return (
        f"# {name}\n"
        "\n"
        f"Cliente: OpenCode. MUST Read `{canonical_dir}/SKILL.md` and follow it as the runbook.\n"
...
def write_stubs() -> None:
    for name, content in expected_stubs().items():
        dest = OPENCODE_SKILLS / name / "SKILL.md"
