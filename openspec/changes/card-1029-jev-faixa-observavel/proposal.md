...
- **A/B da janela de estado.** A **janela de estado maior é produzida pelo produto** por configuração efectiva (a mesma configuração estende a retenção da janela e o horizonte dos agregados enviados ao modelo; o payload declara a janela efectiva) — não é um rótulo. O **braço é derivado da janela efectivamente enviada** (`larger` só existe quando a janela maior foi de facto enviada; a env independente deixa de mandar no rótulo). Os **gates e o limiar são idênticos nos dois braços**; só o contexto enviado difere, por desenho. Uma leitura **read-only** sobre os registos de diagnóstico mede a **confiança média** com a janela atual e com a janela maior, com a **mesma versão fixa de modelo** (pin do #1028), com o **orçamento de payload do #1025 (≤ ~500 tokens) preservado** (o bloco `window` tem forma fixa: sete escalares + no máximo 5 trades), declara o **tamanho da amostra** e só conclui com **≥30 janelas de 900 s não sobrepostas**; abaixo disso escreve **amostra insuficiente**.
