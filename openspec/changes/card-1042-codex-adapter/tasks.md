...
- [x] 4.2 Publicar tag do produto e aplicar o pin no worktree Cripto somente depois do gate humano de Design; antes de escrever, capturar o mapa **destino** vigente e, depois, comparar `juizo.label/slug`, `execucao.label/slug` e `forbid` byte a byte, além de inspecionar diff, overlay e quatro adapters existentes. Evidência: `evidence/qa-closeout.md`; tag `v1.1.19`, preservação byte a byte e revisão final do pin confirmadas. <!-- covenant-flow:after-commit -->
...
- [x] 5.3 Provar no CLI em filhos reais o par `gpt-6-sol/high` e `gpt-6-luna/max`, falha sem fallback para par ausente/indisponível, isolamento de Design/Apply/QA e dois reviewers read-only com diff idêntico. Juízo, Apply, reviews e QA têm evidência de filhos isolados e proxies observados; a sessão QA CLI `01a0db04-661e-7d22-abc1-13e1de8eeb91` observou `gpt-6-luna/max`, completou e retornou payload. Check `process-fsm` cobre as recusas sem fallback. Limite cooperativo registrado: o spawn nativo real ficou fora de `PreToolUse`; Alan aceitou essa cobertura no comentário do #1042. IDE local fora do recorte CLI, não alegada. Evidência: `evidence/qa-closeout.md`, `cli-apply-validation.md` e `review-wave-1818-and-p1-fixes.md`. <!-- covenant-flow:after-qa -->
- [x] 5.4 Percorrer um card piloto pela FSM: Design, aprovação de Alan, Apply, dois reviews, QA e Done técnico; guardar eventos, proxies e evidência de cada gate sem pular etapa. Evidência: `evidence/qa-closeout.md`, check run `qa-gate` green e T14/Status `Done`. <!-- covenant-flow:after-qa -->
- [x] 5.5 Depois do ensaio documentado, atualizar a descrição do board para retirar apenas a afirmação de Codex inativo; registrar explicitamente a cobertura cooperativa e validar OpenSpec/estado final. Evidência: `evidence/qa-closeout.md`, leitura JSON do Project 1 e `openspec validate` válido. <!-- covenant-flow:after-qa -->
