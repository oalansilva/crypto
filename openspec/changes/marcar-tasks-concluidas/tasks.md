...
- [x] 2.1 Modificar `update_task` em `backend/app/routes/workflow.py` para chamar `toggle_task_checkbox` quando `state` muda para `done` ou `queued`
- [x] 2.2 Extrair `task_code` do campo `description` (formato: `code:1.1` ou `code: 1.1`)
- [x] 2.3 Obter `change_id` via lookup do `change_pk` do work item
- [x] 2.4 Garantir que transação só commita se ambas operações (DB + file) succeed; se file write falhar, fazer rollback da DB
- [ ] 2.5 Testar: state=done → checkbox vira [x]; state=queued → checkbox volta para [ ]

## 3. Frontend — TaskTree checkbox interativo

- [x] 3.1 Adicionar prop `onToggle?: (taskCode: string, workItemId: string, checked: boolean) => void` em `TaskTree`
- [x] 3.2 Substituir checkbox passivo (só leitura) por `<input type="checkbox">` com handler
- [x] 3.3 Handler: extrair `work_item_id` dos dados do work item, chamar `PATCH /workflow/work-items/{work_item_id}` com `{ state: checked ? "done" : "queued" }`
- [x] 3.4 Mapear `task_code → work_item_id` usando dados já presentes nos work items (description contém `code:1.x`)
- [ ] 3.5 Mostrar loading state (checkbox desabilitado + spinner) durante chamada API
- [x] 3.6 Em caso de erro: reverter checkbox visualmente, mostrar toast de erro via `toast()` do Shadcn
- [x] 3.7 Invalidar `tasksQuery` e `workItemsQuery` do TanStack Query após sucesso
- [x] 3.8 Não alterar aparência de seções/títulos — só os checkboxes das tasks individuais

## 4. Frontend — Resolver task_code → work_item_id

- [x] 4.1 Work items já contêm `description` com formato `code:1.1` — usar regex para extrair
- [x] 4.2 Criar utilitário `extractTaskCode(description: string): string | null` para parsing
- [x] 4.3 Passar `workItemId` junto com cada task na renderização do TaskTree

## 5. Backend — GET /api/portfolio/kpi

- [x] 5.1 Criar `backend/app/routes/portfolio.py` com router `APIRouter(prefix="/api/portfolio", tags=["portfolio"])`
- [x] 5.2 Registrar o router em `backend/app/api.py` (importar e incluir no `APIRouter`)
...
- [x] 6.1 Adicionar query `portfolioKpiQuery` em `HomePage.tsx`:
  ```ts
  ...
  ```
- [x] 6.2 Definir tipo `PortfolioKpiResponse` com todos os campos do endpoint
- [x] 6.3 Substituir valores hardcoded na KpiCard "Portfolio PnL (hoje)":
...
- [x] 6.4 Substituir valores hardcoded na KpiCard "Drawdown (30d)":
...
- [x] 6.5 Remover `label="valor ilustrativo"` das duas KpiCards (substituir por loading state ou valor real)
- [x] 6.6 Loading state: mostrar `<KpiSkeleton />` enquanto query carrega
- [x] 6.7 Manter as demais KpiCards (Melhor estratégia, Data freshness) funcionando como antes
