...
- [x] 3.4 Com o host no ar: prompt da Q fechada = título + conflito; recomendação só na primeira option `(Recommended)`; MUST NOT copiar `➡️` nem `Recomendada:` + o texto da option para o prompt. Fallback Matt (`➡️` no corpo) só se o host estiver indisponível. N2: `_plain` de `## Cliente: dsh` contém `recomendação vive só` e `não copie`
...
- [x] 5.3 Residual de homologação: commit + tag **`v1.1.4`** (não mover `v1.1.3`); skill `grill-card` sem `➡️` duplicado no prompt do host
...
- [x] 6.3 `implantar --pin v1.1.4` no Cripto; overlay `pin: v1.1.4`; `clients.dsh.auto: false` permanece
