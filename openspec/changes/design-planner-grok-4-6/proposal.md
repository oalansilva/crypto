...
- Introduz o subagent `design-planner` com `model: openai/gpt-5.6-sol` fixo e **effort sempre `high`**, executando o contrato `design-critic` no `Status=Design`.
...
- `design-planner-routing`: roteamento do gate Design para o subagent `design-planner` (GPT 5.6 Sol, effort `high`), coexistência OpenAI/Go, fallback autorizado e restrição de escopo (só artefatos de design/protótipo).
