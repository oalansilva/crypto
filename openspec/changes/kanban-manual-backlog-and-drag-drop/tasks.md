...
- [x] 5.5 Publish/reconcile the pending repo changes so the upstream guard allows promotion from `DEV` to `Alan homologation`; the delivery commit is now published on `origin/main` and the runtime/Kanban state has been reconciled to `Alan homologation`.
