...
- [x] 3.3 Registrar no card os diffs/SHAs das duas camadas e confirmar que local e global não divergem.

## 4. Validação e handoff

- [x] 4.1 Rodar os testes focados dos perfis/skills, parse TOML e validação OpenSpec da change e global.
- [x] 4.2 Executar Code Review independente no diff exato com prova de estado idêntico antes/depois.
- [x] 4.3 Em QA, verificar uma lane Luna natural com metadata observável e auditoria de escopo, sem iniciar agente somente para smoke test.
- [x] 4.4 Publicar evidências finais no card e manter explícito que a opção 2 não oferece isolamento forte do sistema operacional.
