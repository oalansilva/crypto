# `closeout-links` — regra geral: qualquer ligação local vira clicável (fechamento = caso primário)

Tipo: nova capability. Origem: card #872 (história grelhada, fonte da verdade; Q2 REVERTIDA por Alan — regra geral, fechamento caso primário). Escopo: QUALQUER ligação local no chat (caminho, `file://`, porta do painel); fechamento (docs do dia + PRs do pacote) é o caso primário. O comportamento `xdg-open`/`canOpenNativePath` vive no harness dsh (outro checkout) — aqui, contrato observável sobre a mensagem.
