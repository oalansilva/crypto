# OHLCV Timescale Storage Specification

## Scope

This specification covers backend persistence and retrieval for market candles used by monitoring and analysis features.
...
- Write path latency can be above 500ms if upstream provider stalls, but read path from persisted storage MUST be optimized for < 500ms in the normal operating envelope.
- The storage layer SHOULD expose health metrics including:
  - last successful ingest per symbol/timeframe
  - row insert rate
  - skipped duplicates
