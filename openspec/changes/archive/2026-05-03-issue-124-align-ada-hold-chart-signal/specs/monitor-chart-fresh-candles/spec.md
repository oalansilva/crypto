...
The Monitor chart candles endpoint SHALL refresh from the live market provider when persisted OHLCV candles exist but are stale for the requested timeframe.

#### Scenario: Persisted daily candles are stale
- **WHEN** the chart requests `1d` candles for a crypto pair
- **AND** the latest persisted candle is older than the freshness window for `1d`
...
- **AND** persisted candles are within the freshness window for the requested timeframe
