...
No momento do `Done`, o título do card no board SHALL ser igual ao título da issue, ou o card deve conter comentário registrando divergência aprovada. Ao renomear uma issue que possui card no board, o título do card não pode ser editado via API Projects v2; nesse caso, o autor SHALL postar nota de divergência no card com motivo e aprovação. O `release-guard audit` SHALL emitir warn quando o título do board diferir do título da issue sem nota de divergência registrada.
...
#### Scenario: Rename pós-Done sem nota
- **WHEN** a issue é renomeada após o card estar em `Done` e o card no board não recebe nota de divergência
- **THEN** o `release-guard audit` emite WARN de divergência de título board/issue sem nota
- **AND** a nota de divergência é registrada no card antes do fechamento de release

#### Scenario: Rename com nota de divergência
- **WHEN** a issue é renomeada e o card no board recebe nota de divergência (motivo + aprovação)
- **THEN** o `release-guard audit` reconhece a nota e não emite warn para aquele card
