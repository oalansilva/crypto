...
#### Scenario: Common user only sees tiered strategies
- **WHEN** a non-admin user requests Monitor opportunities with no tier filter or `tier=all`
- **THEN** the system evaluates only Tier 1, Tier 2, and Tier 3 strategies
- **AND** untiered strategies are excluded

#### Scenario: Admin can inspect untiered strategies
- **WHEN** an admin user requests Monitor opportunities with `tier=all`
- **THEN** the system preserves the requested `all` filter and can include untiered strategies
