...
## Purpose
Contrato transversal de **emissão** (chat/`design.md`/handoff) vs **avaliação** Impeccable. Snapshot, um chat por coluna, contexto fatiado no apply, folha de tokens, proxies de custo. Não enfraquece gates.
...
The runbook SHALL require a separate chat per column (Design, Apply, Review, Release) titled `#<id> <coluna>` on both Cursor and Grok. The agent MUST refuse to mix those columns in the same transcript. This SHALL NOT add a state, event, hook, or `enabled_tools` change to `.cursor/process-fsm.yaml`.

#### Scenario: Agent refuses a mixed column chat
- **WHEN** a chat titled for Design receives an Apply/Review/Release request
- **THEN** the agent refuses to execute that other column in the same transcript
- **AND** it tells the operator to open a new chat titled `#<id> <coluna>`
- **AND** `process-fsm.yaml` is unchanged

#### Scenario: New column starts a new chat
- **WHEN** the operator starts Apply after Design
- **THEN** that work runs in a chat titled `#<id> Apply` (or the matching column)
- **AND** it does not continue the Design transcript
