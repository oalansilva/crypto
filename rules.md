...
9. Sempre utilizar subagentes quando houver tarefa de desenvolvimento, investigacao, validacao ou revisao tecnica com ganho claro de paralelismo.
   ...
   - **Excecao explicita - planejamento de design:** o gate `Design` (specs, critica e prototipo) usa o subagent `design-planner` com model fixo `openai/gpt-5.6-sol` (OpenAI via OAuth) e effort `high` (segunda excecao, junto do `vision`); critics A/B herdam o modelo da sessao de design designada com igualdade observavel (`BLOCKED` sem ela); frontier nunca roda como sessao principal (envelope de custo ≤ $1,00/card no planner); fallback para `opencode-go/grok-4.5` (effort high) so com autorizacao de Alan.
