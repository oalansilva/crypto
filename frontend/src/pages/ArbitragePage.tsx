...
import { Shuffle, X, Sparkles, Bookmark, Layers, Wallet, Activity, Kanban } from 'lucide-react'
        ...
        <section className="glass rounded-2xl border border-zinc-800 overflow-hidden">
          <div className="px-6 py-4 border-b border-zinc-800 flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-zinc-400">Exibe todos os pares (mesmo abaixo do threshold).</p>
            </div>
          ...
          <div className="space-y-6 p-6">
                      ...
                      <table className="w-full text-sm text-left text-zinc-400">
                        ...
                        <tbody>
                          {spreads.map((item, index) => (
                            <tr key={`${sym}-${item.buy_exchange}-${item.sell_exchange}-${index}`} className="border-b border-zinc-800">
                              ...
                              <td className="px-4 py-3">
                                {item.meets_threshold ? (
                                  ...
                                  <span className="text-slate-500">abaixo do threshold</span>
                                )}
                              </td>
            ...
            {!data?.symbols?.length && !loading && (
              <div className="text-center text-slate-500">Nenhum spread encontrado no momento.</div>
            )}
          </div>
        </section>
      </div>
