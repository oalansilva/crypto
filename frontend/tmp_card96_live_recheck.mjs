...
await context.addInitScript((payload) => {
  ...
  localStorage.setItem('auth_user', JSON.stringify({ id: payload.id, email: payload.email, name: payload.name, isAdmin: payload.isAdmin }));
}, auth);
...
const summary = [`URL: ${page.url()}`,'Observed via Playwright on live UI with dev login.','BTC card text:',cardText,'','BTC modal text:',modalText,'','Network:',JSON.stringify(network, null, 2)].join('\n');
