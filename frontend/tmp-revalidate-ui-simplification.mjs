...
const OUT = path.resolve('../qa_artifacts/playwright', FLOW);
...
const artifact = (name) => ({ path: path.join(OUT, name), url: `${BASE_URL}/qa-artifacts/playwright/${FLOW}/${name}` });
const writeJson = async (name, data) => { const target = artifact(name); await fs.writeFile(target.path, JSON.stringify(data, null, 2)); return target; };
const shot = async (name, locator = page) => { const target = artifact(name); await locator.screenshot({ path: target.path }); return target; };
...
try {
  ...
  checks.push(
    { name: 'card does not show Runtime stage / Stage atual', ok: !/Runtime stage|Stage atual/i.test(cardText) },
    { name: 'card does not show QA functional', ok: !/QA functional/i.test(cardText) },
    { name: 'card shows Publish', ok: /Publish/i.test(cardText) },
    { name: 'card shows Ready for homologation', ok: /Ready for homologation/i.test(cardText) },
    { name: 'drawer does not show Runtime stage / Stage atual', ok: !/Runtime stage|Stage atual/i.test(drawerText) },
  ...
  const summary = {
    ...
    passed,
    checks,
    runtime: { status: changeRes.status, boardColumn: boardItem.column, boardStatus: boardItem.status },
    artifacts: {
  ...
  const summary = { flow: FLOW, changeId: CHANGE_ID, passed: false, error: String(error?.stack || error), failureScreenshot: failureShot?.url || null, checks, commentBody };
  await writeJson('08-summary.json', summary);
  console.log(JSON.stringify(summary, null, 2));
  process.exitCode = 1;
} finally {
