...
test.describe('transparência em Favoritos', () => {
  ...
  test('mostra resumo, overlays e detalhes técnicos sem duplicar o gráfico', async ({ page }) => {
    ...
    const operationDecision = page.getByTestId('result-trades').getByRole('button', { name: 'Ver decisão da operação' }).first()
    await expect(operationDecision).toBeVisible()
    await expect(page.getByRole('button', { name: 'Entenda este trade' })).toHaveCount(0)
    await operationDecision.click()
    await expect(page.getByTestId('result-trades').getByTestId(/-permanent-rules$/)).toHaveCount(0)
