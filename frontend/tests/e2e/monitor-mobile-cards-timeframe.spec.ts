...
test('monitor modal shows recent entry and exit history from the strategy payload', async ({ page }) => {
  ...
  const openButton = openExplanation.getByRole('button', { name: 'Ver decisão da operação' })
  await expect(openButton).toHaveAttribute('aria-expanded', 'false')
  await openButton.focus()
  await page.keyboard.press('Enter')
  await expect(openButton).toHaveAttribute('aria-expanded', 'true')
  await expect(openExplanation.getByTestId('trade-mobile-open-0-permanent-rules')).toHaveCount(0)
  ...
  await closedExplanation.getByRole('button', { name: 'Ver decisão da operação' }).click()
  await expect(closedExplanation.getByTestId('trade-mobile-1-permanent-rules')).toHaveCount(0)
