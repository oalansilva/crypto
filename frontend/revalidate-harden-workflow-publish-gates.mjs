...
const out = path.resolve('../qa_artifacts/playwright/harden-workflow-publish-gates-live');
await fs.mkdir(out, { recursive: true });
const browser = await chromium.launch({ headless: true, chromiumSandbox: false });
const page = await browser.newPage({ viewport: { width: 1440, height: 900 } });
await page.goto('http://72.60.150.140:5173/kanban', { waitUntil: 'networkidle' });
await page.screenshot({ path: path.join(out, '02-board-live.png'), fullPage: true });
await page.getByRole('button', { name: /Open details for harden-workflow-publish-gates/i }).click();
await page.waitForLoadState('networkidle');
await page.screenshot({ path: path.join(out, '03-drawer-live.png'), fullPage: true });
...
await fs.writeFile(path.join(out, '04-live-ui-payload.json'), JSON.stringify(payload, null, 2));
await browser.close();
